phina.define("SpriteCharacter", {
  ...
  init: function(key, animation) {
    ...
    // スプライト描画
    this.superInit("char" + this.key, animation, CHARACTER[this.key].x, CHARACTER[this.key].y, CHARACTER[this.key].char_size, CHARACTER[this.key].char_size);
    // 初期位置
  ...
  updatelabel: function(app) {
    let label = ASSETS.spritesheet["char" + this.key].animations[this.animation].label;
